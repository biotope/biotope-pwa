...
function executeWithInput(processPath, args = [], inputs = [], opts = {}) {
  ...
  const { env = null, timeout = 350, maxTimeout = 10000 } = opts;
  ...
  const loop = inputs => {
    ...
    if (!inputs.length) {
      ...
      killIOTimeout = setTimeout(() => {
        console.error('Reached I/O timeout');
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    childProcess.stderr.once('data', err => {
      ...
      console.log('REJECTING', err.toString());
...
module.exports = {
  ...
  out: {
    removeLines: (times) => `${CODES.CLEAR_LINE}${CODES.REMOVE_LINE}`.repeat(times - 1) + CODES.CLEAR_LINE,
    setGreen: () => CODES.GREEN
  }
};